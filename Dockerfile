...
# System deps for Playwright/Chromium (rarely changes → cached layer)
...
# Install CLIs (rarely changes → cached layer)
...
# Dependencies (changes only when requirements.txt changes → cached)
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Playwright browser (changes only when playwright version changes → cached)
RUN playwright install chromium

# App code (changes every deploy — this layer is small and fast)
COPY main.py .
COPY static/ static/
COPY tests/ tests/
