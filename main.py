...
【最重要】コードは ```python ブロック``` で書けば自動的にE2Bサンドボックスで実行されます。
ユーザーが「実行して」「計算して」「表示して」と言った場合は、必ず実行可能なPythonコードを書いてください。
外部ライブラリ（pandas, numpy, requests等）もE2Bサンドボックスで利用可能です。

回答ルール:
- 必ず動作するコードを ```python ブロック``` で1つ提供する（複数ブロックに分けない）
- コードの前に「何をするコードか」を1行で説明する
- printで結果を出力する（returnではなくprint）
...
## 使えるツール
1. **Pollinations.ai**（無料・APIキー不要）— image_generateツール
2. **DALL-E 3**（OPENAI_API_KEY必要）— より高品質
...
## プロンプトのコツ
- 日本語で指示されても英語に翻訳してから生成
- スタイル指定: realistic, anime, watercolor, flat design, 3D render, etc.
- 用途に合わせたサイズ: SNSバナー(1200x630), Instagram(1080x1080), ポスター(768x1024)""",
    },
    "video": {
        "name": "🎬 動画生成AI",
        "color": "#f43f5e",
