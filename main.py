...
- site_publisher: Webサイト・Webアプリ公開・「XXX.chatweb.aiで公開」・HTML生成してホスティング・「サイトを作って公開」・「ページを公開して」・LP作成・ポートフォリオ
...
async def _auto_backup_db():
    ...
    while True:
        ...
        try:
            ...
            log.info(f"DB backup saved: {dest}")
    ...
    try:
        ...
        for user_id, key_name, key_value in rows:
            ...
            if key_name == "google_access_token":
                _google_tokens[user_id]["access_token"] = key_value
            elif key_name == "google_refresh_token":
                _google_tokens[user_id]["refresh_token"] = key_value
