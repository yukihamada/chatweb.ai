...
UPLOADS_DIR        = "/tmp/synapse_uploads"
...
async def tg_send(chat_id: int | str, text: str, parse_mode: str = "Markdown") -> None:
    # Telegram message limit is 4096 chars
    if len(text) > 4000:
        text = text[:3997] + "..."
    await tg_api("sendMessage", chat_id=chat_id, text=text, parse_mode=parse_mode)
...
async def setup_telegram_webhook(base_url: str) -> None:
    ...
    r = await tg_api("setWebhook", url=webhook_url, allowed_updates=["message"])
    if r.get("ok"):
...
async def process_telegram_message(chat_id: int | str, user_text: str, username: str = "") -> None:
    """Route a Telegram message through Synapse agents and stream reply via message edits."""
    session_id = f"tg_{chat_id}"
    await tg_typing(chat_id)

    # Send placeholder message to get a message_id for streaming updates
    sent = await tg_api("sendMessage", chat_id=chat_id, text="⏳ 考え中...", parse_mode="Markdown")
...
async def process_line_message(user_id: str, text: str, reply_token: str = "") -> None:
    """Route LINE message through Synapse agents."""
    ...
    try:
        ...
        if reply_token:
            ...
            await line_push(user_id, reply_text)

    except Exception as e:
        log.error(f"LINE message processing error: {e}")
        err_text = f"⚠️ エラーが発生しました: {e}"
        if reply_token:
            await line_reply(reply_token, err_text)
        else:
            await line_push(user_id, err_text)
...
async def tool_zapier_trigger(event: str, data: dict) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as hc:
            r = await hc.post(ZAPIER_WEBHOOK_URL, json={"event": event, "data": data, "source": "synapse"})
...
# ══════════════════════════════════════════════════════════════════════════════
_WORKSPACE_ROOT = os.getenv("AGENT_WORKSPACE", "/tmp/synapse_workspace")
...
async def tool_scaffold_and_deploy(
    ...
    safe_name = re.sub(r"[^a-z0-9-]", "-", app_name.lower())[:30].strip("-") or "synapse-site"
...
<body>
  ...
  <section id="features">
    ...
    <div class="grid3">
      ...
      <div class="card">
        <div class="card-icon">💬</div>
        <h3>Telegram統合</h3>
        <p>@synapse_aichat_botからAIエージェントへアクセス。ストリーミング応答対応。</p>
      </div>
      <div class="card">
        <div class="card-icon">💾</div>
        <h3>長期記憶システム</h3>
  ...
  <div class="cta-section" id="contact">
    ...
    <div class="cta-group" style="justify-content:center">
      <a href="https://synapse-demo.fly.dev" class="btn btn-primary" target="_blank">デモを試す →</a>
      <a href="https://t.me/synapse_aichat_bot" class="btn btn-outline" target="_blank">Telegram Bot</a>
    </div>
  </div>
...
"""
    ...
    tmpdir = tempfile.mkdtemp(prefix="synapse_deploy_")
...
async def tool_csv_to_db(file_id: str, table_name: str = "data", session_id: str = "") -> str:
    ...
    if not file_info:
        return f"File not found: {file_id}"
    import sqlite3
    db_path = f"/tmp/synapse_sql_{session_id}.db"
    ...
    try:
        ...
        flow = Flow.from_client_config(
            {"web": {
                ...
                "token_uri": "https://oauth2.googleapis.com/token",
                "redirect_uris": [f"https://{os.getenv('FLY_APP_NAME','localhost')}.fly.dev/auth/google/callback"],
            }},
            scopes=GOOGLE_OAUTH_SCOPES,
        )
        flow.redirect_uri = f"https://{os.getenv('FLY_APP_NAME','localhost')}.fly.dev/auth/google/callback"
...
【Synapse について】
あなたが動いているシステム「Synapse」は、複数のAIエージェントが協調するマルチエージェントプラットフォームです。
- URL: https://synapse-demo.fly.dev
...
- 「Synapseとは？」と聞かれたらこの情報を基に回答してください。
...
GCAL_SERVICE_ACCOUNT_JSON が未設定の場合はセットアップ手順を案内してください。""",
    },
    "drive": {
        "name": "📂 Driveエージェント",
        "color": "#facc15",
...
async def auth_register(req: RegisterRequest):
    ...
    if "test" in email:
        ...
        app_url = os.getenv("APP_URL", "https://synapse-demo.fly.dev")
...
async def set_user_secret(req: SecretSet, request: Request):
    ...
    async with db_conn() as db:
        await db.execute(
            ...
            (sid, user["id"], req.key_name, req.key_value))
        await db.commit()
...
- research: それ以外全般・「〜について教えて」・Synapse自身について・このシステムについて
    ...
    if agent_id == "research":
        ...
        if re.search(r'デプロイ|deploy|上げ|アップ|リリース|反映', message, re.I):
            ...
            if re.search(r'作[るっ]|作[っ]て|作成|新[しく]|ゼロから|generate|create.*site|サイト.*作', message, re.I):
                ...
                gen_name = f"synapse-site-{uuid.uuid4().hex[:6]}"
                if name_hint:
        ...
        if re.search(r'作成|追加|create|add|予定を入れ', message, re.I):
            ...
            results["gcal_list"] = await tool_gcal_list()
...
async def execute_agent(agent_id: str, message: str, session_id: str,
    ...
    # Support image input (vision): screenshot takes priority over uploaded image
    active_image_b64 = screenshot_b64 or image_data
    active_media_type = screenshot_media_type if screenshot_b64 else "image/jpeg"
    ...
    async def run():
        try:
            ...
            memories, plan = await asyncio.gather(
                search_memories(req.message, limit=6),
...
async def agent_card():
    base_url = f"https://{os.getenv('FLY_APP_NAME','synapse-demo')}.fly.dev"
    tools = []
        ...
        async with db_conn() as _db:
            for kname, kval in [("google_access_token", creds.token or ""),
                ...
                if kval:
                    await _db.execute(
                        "INSERT OR REPLACE INTO user_secrets(id,user_id,key_name,key_value) VALUES(?,?,?,?)",
                        (uuid.uuid4().hex[:16], uid, kname, kval))
            await _db.commit()
...
async def telegram_webhook(request: Request):
    ...
    if text == "/start":
        await tg_send(chat_id,
            ...
            "何でも聞いてください — リサーチ、コード、分析、デプロイまで対応します。\n\n"
            "📖 `/help` でコマンド一覧を表示")
        return {"ok": True}
    ...
    if text == "/memory":
        ...
        if mems:
            ...
            await tg_send(chat_id, "*記憶一覧:*\n" + "\n".join(lines))
        else:
            await tg_send(chat_id, "まだ記憶がありません。")
        return {"ok": True}
    ...
    if text == "/clear":
        ...
        async with db_conn() as db:
            ...
            await db.commit()
        await tg_send(chat_id, "✅ 会話履歴をリセットしました。")
        return {"ok": True}
...
async def execute_agent_extended(agent_id: str, message: str, session_id: str = "",
    ...
    if agent and agent_id in ("agent_creator", "platform_ops"):
        ...
        if platform_results:
            ...
            message = f"{message}\n\n---\n{tool_context}"
    return await _orig_execute_agent(agent_id, message, session_id, history, memory_context, image_data)
